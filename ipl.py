...
def search_club():
    a = entry.get()
    query = "SELECT winner, COUNT(winner) from ipl_winners where winner like %s group by winner;"
    para = ('%' + a + '%',)
...
def year_winners():
    b = entry.get()
    query = "select winner,runner_up from ipl_winners where years=%s;"
    para = (b,)
    cursor.execute(query, para)
...
def runners_club():
    c = entry.get()
    query = "SELECT runner_up, COUNT(runner_up) from ipl_winners where runner_up like %s group by runner_up;"
    para = ('%' + c + '%',)
    cursor.execute(query, para)
    ...
    if result:
        for record in result:
            runnerup = record[0]
            count = record[1]
            query_years = "select years from ipl_winners where runner_up = %s"
            cursor.execute(query_years, (runnerup,))
            years_result = cursor.fetchall()
            years_won = ','.join(str(year[0]) for year in years_result)
            result_text.insert(tk.END, f"Club Name : {runnerup}\nNumber of times IPL Runner up: {count}\nYears team came Second: {years_won}\n")
    else:
        result_text.insert(tk.END, "No result found.")


def link():
    webbrowser.open_new(r"https://www.iplt20.com/")
...
def newentry():
    ...
    #    cursor=mydatabase.cursor()
        ...
        query="insert into ipl_winners(years,winner,runner_up) values(%s,%s,%s)"
...
mydatabase = m.connect(host="localhost", user="root", password="12345", database="ipl")
cursor = mydatabase.cursor()

window = tk.Tk()
window.title("ipl Search App")
...
label = tk.Label(input_frame, text="Enter team name, years, or runner-up:", bg="purple", fg="white",font=("Arial", 14, "bold"))
...
logo_image = ImageTk.PhotoImage(Image.open("Tata Ipl 2022 Logo Png Images - 1280x768.png").resize((600, 150)))
...
Mainsite = tk.Button(window, text="IPL Website", command=link, font=("Arial", 12, "bold"), bg="blue", fg="white")
